...
void setupHCSR04()
{
    TIMSK1 |= (1 << ICIE1); // Changed TMSK1 to TIMSK1
    TCCR1A = 0;
    TCCR1B |= (1 << CS10);
    TCCR1B &= ~(1 << CS11);
    TCCR1B &= ~(1 << CS12);
...
public:
    ...
    HCSR04(char triggerPort, uint8_t triggerBit, uint8_t echoInterrupt): HCSR04()
    {
...
ArrayList<HCSR04> HCSR04s;

void endTrigger()
{
    for (int i = 0; i < HCSR04s.getSize(); i++)
    {
